...
```{r echo = TRUE, message = FALSE, warning = FALSE, comment=NA}
train$is_in_collection <- unlist(lapply(train$belongs_to_collection, function(collection) ifelse(collection == "", 0, 1)))
train$belongs_to_collection <- NULL

test$is_in_collection <- unlist(lapply(test$belongs_to_collection, function(collection) ifelse(collection == "", 0, 1)))
...
The number of movies is negligible for crews having more than 3 directors. We see that only `r directors$number_of_movies[directors$number_of_directors == 0]` movies with crews having no directors have been made but generated big average revenues compared to the others having at least a director. Is this an enough large sample to conclude that the revenue will be much better if the crew has no director, since it represents only `r directors$number_of_movies[directors$number_of_directors == 0] / nrow(train) * 100`% of the movies in the train dataset? On a mathematical point of view, the sample is not enough large to conclude but on a data driven point of view, those movies show that having no directors in the crew implies bigger revenues. Furthermore, most of movies (`r directors$number_of_movies[directors$number_of_directors == 1] / nrow(train) * 100`% of them) have been made with one director in the crew. 
