...
```{r echo = TRUE, message = FALSE, warning = FALSE, comment=NA}
## Step 1: Extract the genres of a movie.
...
AppendExtractedData <- function(dataset, feature, subfeatures.to_exclude)
{
...
## Step 2: Append the data extracted.
...
## Step 4: Bar plot of the average revenue based on the genre.
movies.genres %>%
...
According to these bar plots, both of them show that the following genres generate the highest revenue:
...
for(i in seq(1:train.length))
{
    if(train[i, "genres"] != "")
    {
        genres <- fromJSON(train[i, "genres"])
...
train$best_genre <- genre.best_revenue
...
4. Show a bar plot of the average revenue in function of the spoken language.
5. Show a bar plot of the number of movies in function of the spoken language.
...
movies.spoken_languages <- movies.spoken_languages %>%
    ...
    summarise(mean_revenue = mean(revenue),
              number_of_movies = n())
#movies.spoken_languages$id <- seq(1, nrow(movies.spoken_languages))

## Step 4: Bar plot of the average revenue based on the spoken language.
...
## Step 5: Bar plot of the number of movies based on the spoken language.
movies.spoken_languages %>%
...
According to this bar plot, the revenue is considerably greater for movies having a homepage than the ones that do not.
...
GetLanguageIDFromName <- function(language)
{
    return(original_languages$id[original_languages$name == language])
}

train$original_language <- unlist(lapply(train$original_language, GetLanguageIDFromName))
...
According to this bar plot, our assumption holds.
...
3. Show bar plots of the revenue in function of the release year, release month and release week-day.
...
## Step 3: Show bar plots of the revenue in function of the release year, release month and release week-day.
train %>%
