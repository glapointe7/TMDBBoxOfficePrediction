...
knitr::include_graphics("Images/TMDB_Box_Office_Prediction.svg")
```
...
train %>%
    ...
    summarise(mean = mean(revenue)) %>%
    ggplot(aes(x = number_of_crew_members, y = mean)) +
          geom_bar(stat = "identity") + 
...
According to this bar chart, having directors in the crew have a significant negative impact on the movie revenue. It may be caused by a selection of inexperienced member of the crew or actors. It may also be caused by the downfall of the director who was too confident because he did a good movie once. Many reasons can explain this negative impact.
...
library(Metrics)
cat("RMSLE =", rmse(revenue.estimated, log(train$revenue + 1)), "\n\n")
```
...
$$
This is the RMSE (Root Mean Squared Error) loss function where $f(x_i) = \ln(\hat{y} + 1)$ and $y = \ln(y + 1)$. We obtain the gradient:
$$
\begin{align}
    \nabla F(\hat{y}, y) &= \frac{\partial F(\hat{y}, y)}{\partial \hat{y}} \\
                         &= \frac{1}{2}\frac{\partial (\ln(\hat{y} + 1) - \ln(y + 1))^2}{\partial \hat{y}} \\
                         &= \frac{\ln(\hat{y} + 1) - \ln(y + 1)}{\hat{y} + 1}.
\end{align}
$$
We deduce the hessian from the gradient:
$$
\begin{align}
    \nabla^2 F(\hat{y}, y) &= \frac{\partial^2 F(\hat{y}, y)}{\partial \hat{y}^2} \\
                         &= \frac{\partial \frac{\ln(\hat{y} + 1) - \ln(y + 1)}{\hat{y} + 1}}{\partial \hat{y}} \\
...
### Predictions
The objective is to apply our optimal model to our test dataset in order to calculate and obtain the predictions on the movies revenue in a CSV file.
```{r echo = TRUE, message = FALSE, warning = FALSE, comment=NA}
test.matrix <- xgb.DMatrix(data.matrix(test))

prediction.test <- as.integer(exp(predict(model, test.matrix)) + 1)
submission <- data.frame(id = movies.id,
                         revenue = prediction.test)
write.csv(submission, "Submissions/Submission_XGBoost.csv", row.names = FALSE)
```


## Random Forest Model
The Random forest model is part of the ensemble set of models like the gradient boosting trees one. This model uses decision trees as weak learners in order to build its forest of $n$ decision trees. 
